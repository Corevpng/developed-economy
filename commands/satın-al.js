...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('satın-al')
    .setDescription('Marketten bir ürünü satın alır.')
  ...
  async execute(interaction) {
    ...
    if (!item) {
      return interaction.reply({ content: 'Bu ürün markette bulunmuyor. 🚫', ephemeral: true });
    }

    if (!moneyData[userId] || moneyData[userId].balance < item.price) {
      return interaction.reply({ content: 'Yeterli pcoin bulunmuyor. 😔', ephemeral: true });
    }
